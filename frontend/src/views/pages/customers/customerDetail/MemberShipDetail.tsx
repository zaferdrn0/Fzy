...
const CustomerDetail: React.FC<CustomerDetailProps> = ({ customer }) => {
    ...
    // Hizmet dağılımı için veri
    const servicesDistribution = useMemo(() => {
        const distribution = customer.services.map(service => ({
            name: capitalizeFirstLetter(service.type),
            value: customer.subscriptions.filter(sub => sub.serviceId === service._id && sub.isActive).length,
        }));
        return distribution;
    }, [customer.services, customer.subscriptions]);

    // Ödemeler aylık dağılımı için veri
